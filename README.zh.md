...
来自一次真实会话——411 个模型步、26 次折叠，中途升级过一次宿主——数字直接读自 harness 自己的用量记录：

| 项目 | 实测 |
| --- | --- |
| 被折叠摘要替换掉的原始历史 | **441,100 token**（26 次折叠） |
| 主对话的提示缓存命中率 | **99.1%**（52,127,098 token 提示，仅 483,962 未命中） |
| 折叠摘要调用的缓存命中率 | **68.0% → 98.1%** |
| 摘要调用重复付费的 span token | **消除 216,490**（15 次折叠；未命中降到 47,710，只剩尾部指令） |
| 为生命周期提示重发的 runtime-context 快照 | **24 → 0**（4,612 → 910 token） |

折叠那两行为什么会变：`dsh` ≥ `0.1.5-alpha.1` 把系统提示词搬进了 `messages[0]`，与折叠信封撞车，摘要调用的前缀缓存因此失效——每次折叠都把整个 span 重付一遍。修复后，一个 40,422 token 的 span 只需 4,695 未命中（尾部指令），而此前要重付全部 40,422。折叠掉的历史也不再被重发，所以主对话在 411 个步里保持了 99.1% 的缓存命中。

一次会话、一种任务形态——你的数字会不同；关键是机制本身：完成的工作离开表层，稳定前缀持续命中。
