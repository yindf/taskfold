...
- 开启的任务是**命名派生态**：`taskMarks` 会话投影只折叠宿主原生事件——工具调用块注册待定意图，工具结果文本（`Task begun: NAME` / `Task folded: NAME`；legacy 的 `Task ended: NAME` 同样解析）按名压栈/弹栈。按名关闭不可能破坏其他任务；失败的 `task_fold` 不改变任何状态（原子结束即折叠）。
