...
- **rc 通道 —— 支持到 `0.1.5-rc.1`**（2026-09-10 在 0.32.1 上实测：离线测试套件 131 条断言全绿；对本插件所 import 的十个 `@deepseek-ai` 包做逐字节接缝审计，实现文件与先前验证过的 `0.1.5-alpha.2` 构建逐字节一致，仅各自 `package.json` 的版本字段不同；以及在运行中的 0.1.5-rc.1 宿主上活体折叠，挂载副本与本仓库发布提交逐字节一致——4/4 次折叠通过 `verify-cache --since-restart`（前缀缓存命中 97.5–98.2%），磁盘归档与 `fold_recall` 对同一 span 切片逐行同号，对同一份会话日志的 15 次折叠回放显示位置起点在每一次折叠上都成立。本轮无需改代码；它所取代的 rc 记录——`0.1.2-rc.1`——曾抓出并修复一个并行 `task_begin` 的折叠起点缺陷）。`dsh`、`dsh-compaction-basic`、`dsh-llm` 三者版本锁步发布，一个数字覆盖全部耦合面。
