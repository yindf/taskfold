...
export default {
  ...
  apply(ctx) {
    ...
    const taskBegin = {
      name: 'task_begin',
      description: 'Begin a NAMED task. The name is the identity; when the work is done, one task_end({ name }) call ends it and queues archival — the span folds automatically at the next step boundary after the assistant message that follows your task_end result (any content opens the gate — make it your report). A name already open is rejected; names must not contain " —" (a space followed by an em dash). Tasks can nest: task_begin while a task is open opens a subtask (innermost closes first). The call message (with its opening reasoning) stays live in the transcript as the task\'s bookmark; the eventual fold\'s archive starts just after the \'Task begun\' result — the result itself stays live beside the call. This MUST be the only task-mark call in its message: a message that also carries another task_begin/task_end call is rejected at execute time — re-issue it alone in the next message.',
      parameters: {
        type: 'object',
        properties: {
