...
const FOLD_SUMMARY_CORE = [
  ...
  '## What happened',
  '- [the work performed in this span, in order — one bullet per intent (one coherent purpose or phase, typically 3-5 steps), each bullet citing its L<N>-<M> copied from the Span message index AND the reasoning behind the work: why this path, which alternatives were rejected and why]',
  ...
  '- Granularity (What happened): a step = one assistant action together with its tool results (typically 2-4 span lines). A bullet covers ONE intent — one coherent purpose or phase of work; 3-5 steps is the typical size, a smaller intent still gets its own bullet, and 10 steps is the hard ceiling — a bullet that would exceed it MUST be split into consecutive bullets, each keeping its own L<N>-<M>. Never join distinct intents into one bullet — not even with separators. Coverage is a correctness requirement: when detail must compress, write terser bullets — never fewer bullets, never merged, never dropped.',
  '- Reasoning (What happened): the span\'s thinking blocks are the primary source of decision rationale — hypotheses weighed, options compared, why one path was chosen over another, what a result confirmed or refuted. Every What happened bullet ENDS with an explicit why: clause naming the deciding consideration, plus the strongest rejected alternative when one existed — an action-only bullet is a failure. When the span already contains a polished report, do not merely echo it: capture the deliberation that produced it — hypotheses discarded mid-work, uncertainty resolved, why steps ran in that order. Distinguish settled conclusions from passing guesses. Failure causes belong in Pitfalls & gotchas; choice rationale belongs here.',
