...
| `task_end` | Always closes the innermost task — execute only transitions state and returns immediately. A follow-up fold (triggered when this result lands in the log) then compresses the COMPLETE task — `task_begin` pair, body, and the `task_end` pair — into one summary node, leaving a single node instead of four residual lifecycle nodes; the summarizer sees the task's whole lifecycle, so the summary carries no stale "call task_end" pending. If the span is too small the history is left as-is. Optional `title` (short imperative name) labels the fold in `compact_stats`/`compact_recall` listings. |
