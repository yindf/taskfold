...
- **rc channel — supported through `0.1.5-rc.1`** (verified 2026-09-10 on 0.32.1: the offline suite — 131 assertions, all green; a byte-level seam audit of the ten `@deepseek-ai` packages this plugin imports, whose implementation files are byte-identical to the previously verified `0.1.5-alpha.2` build, only their `package.json` version fields differ; and live folding on the running 0.1.5-rc.1 host, where the mounted plugin copy is byte-identical to this repo's release commit — 4/4 folds pass `verify-cache --since-restart` (97.5–98.2% prefix-cache hit), the artifact on disk and `fold_recall` number the same span slice line for line, and a 15-fold replay of one session log shows the positional region start holding on every fold. No code change was needed; the rc record it supersedes — 0.1.2-rc.1 — caught and fixed a parallel-`task_begin` fold-start bug). `dsh`, `dsh-compaction-basic`, and `dsh-llm` ship version-locked, so one number covers the whole surface.
