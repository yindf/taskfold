...
- **alpha channel — supported through `0.1.7-alpha.1`** (verified 2026-09-22 on 0.34.7: the offline suite — 194 tests, all green; a host-API probe against the real `dsh-compaction-basic`/`dsh-llm` packages — 12/12, covering the engine class export, the `summarize`/`compactRegion` prototypes, `BlockAssembler` on the current chunk grammar, and a `ScopedEngine` end-to-end fold with the prefix-anchored envelope, single system message, constructed heading, and fenced span index with true line numbers; a source audit of all nine runtime seam surfaces in the installed host (`agent/pre-step` waterfall, `agent/turn-stopping` serial, `sessionProjections.register`/`stateOf` — its package is byte-identical this round, `systemPrompt.section`/`context`, `tools.register`, the session surface API, and the compaction discriminator); and a live fold on a real 0.1.7-alpha.1 session — a one-shot headless twin of the web profile running the mounted fixed build — where a full task cycle (begin → parallel reads → end → text-only report) folded at turn end and passed `verify-cache`: 89.8% prefix-cache hit, negative tail. This upgrade bit: 0.1.7-alpha.1 ships session format v4, whose `tool/result` events are flattened from nested tool-result blocks to tool-role messages with a message-level `toolCallId` (resumed v3 logs are migrated in place, so v4 is the only live grammar); every result-text reader in the plugin parsed those events as empty — pending intents registered but marks never pushed, `task_end` saw `(none)` open, and no fold ever queued while the host's own pressure compaction did all the work (observed live on the running host before the fix). All result readers now go through one shape-independent extraction (`toolResultEntries`: v4 message-level linkage, v3 block-level for captured logs) — the taskMarks reducer, the deferred-archive plan's guards and begun-result scan, legacy fold titling, lifecycle aging, and span previews, where v4 tool messages render as correlated `tool: ←…` lines and the retired `plugin` source kind generalizes to any non-user producer kind (`harness:`). The engine/assembler seams the fold rides are unchanged — `summarize(input, agent, signal)`, the recover-loop waterfall delegation, `buildSummarizationInput`'s `{tools, messages: [system, …]}`. The record it supersedes — `0.1.6-alpha.2` — stood as written).
