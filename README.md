...
- **rc channel — supported through `0.1.7-rc.1`** (verified 2026-09-24 on 0.35.2 — the release that merges the alpha channel's whole functional line through v0.35.1 onto master, byte-exact: `git diff <alpha-HEAD> HEAD -- plugins test` is empty. The offline suite — 14 suites, 196 tests, 0 fail; a host-API probe run against this branch's `fold-engine` copy and the real rc.1 packages — engine class export, ScopedEngine subclassing the actual rc.1 base class, `BlockAssembler`, and the end-to-end fold path (prefix-anchored envelope with host-head dedup, exactly one system message, the fenced line-numbered span index, usage passthrough) — 12/12, including the 0.1.6-alpha.2 seam adaptations (recover-loop waterfall, routed-config spread) now carried here; and live folding on the running 0.1.7-rc.1 host — 6/6 folds pass `verify-cache --since-restart` (93.9–96.3% prefix-cache hits), produced by the mounted copy whose plugins files hash-match alpha's v0.34.7 release commit — i.e. this release's merged functional code, exercised live. The host-jump byte audit and client-contract sweep behind `0.1.7-rc.1` stand as recorded in the 0.34.8 CHANGELOG entry. One dist-tag note: `0.1.7-rc.1` ships under `next` while `latest` still resolves to `0.1.5-rc.3`, so a bare `npx @deepseek-ai/dsh web` keeps running rc.3 — only `@0.1.7-rc.1` (or `@next`) reaches the new build). `dsh`, `dsh-compaction-basic`, and `dsh-llm` ship version-locked, so one number covers the whole surface.
...
- **Bounds: enforced floor, untested ceiling.** Since this release the plugin declares its host surface in `peerDependencies` (`@deepseek-ai/dsh`, `@deepseek-ai/dsh-compaction-basic`, `@deepseek-ai/dsh-llm`, all `^0.1.7-rc.1`): dsh 0.1.7 hosts check those ranges at install and at startup/recomposition, so an incompatible host now gets the bundle skipped with an `incompatible-version` verdict instead of loading it. Hosts older than the check itself negotiate nothing — on those, folds degrade (tasks still close, unfolded) rather than corrupt. After each dsh upgrade, re-check this section and update it with test results.
