...
- **alpha channel — supported through `0.1.6-alpha.2`** (verified 2026-09-18 on 0.34.6: the offline suite — 188 tests, all green; a host-API probe against the real `dsh-compaction-basic`/`dsh-llm` packages — 12/12, covering the engine class export, the `summarize`/`compactRegion` prototypes, `BlockAssembler` on the current chunk grammar, and a `ScopedEngine` end-to-end fold with the prefix-anchored envelope, single system message, constructed heading, and fenced span index with true line numbers; a source audit of all nine runtime seam surfaces in the installed host (`agent/pre-step` waterfall, `agent/turn-stopping` serial, `sessionProjections.register`/`stateOf`, `systemPrompt.section`/`context`, `tools.register`, the session surface API, and the compaction discriminator); and live folding on the running 0.1.6-alpha.2 host, where the mounted plugin copy is byte-identical to this repo's HEAD blobs — folds pass `verify-cache --since-restart` (89.7–90.6% prefix-cache hit, negative tail). The upgrade required two code changes, both live-verified: the host's `summarizeCompaction` gained an error recover loop that calls `ctx.waterfall('compaction/summary-error', …)` — the shim ctx now delegates that to the real Context (a scoped-fold summarize failure had surfaced as a masking `TypeError`, observed live as one failed fold before the host's own pressure compaction swallowed the span) — and the zai/glm adapter now threads `reasoningEffort` into the request's token stream, so fold options mirror `buildRequest()`'s full routed config (without it the fold diverged from every main request right after the system+tools block: 48.0% hit, tail +8444). The record it supersedes — `0.1.5-alpha.2` — stood as written).
