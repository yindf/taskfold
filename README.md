...
| `task_end` | Always closes the innermost task. If its span is large enough, it folds the task_begin pair AND the body into one summary node inline (the closing task_end pair necessarily stays, and its result neutralizes any stale "call task_end / open marks" lines the summary may carry — the summarizer cannot see the very call that produces it). Too-small spans end without compaction. Optional `title` (short imperative name) labels the fold in `compact_stats`/`compact_recall` listings. |
